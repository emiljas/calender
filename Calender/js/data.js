...
(function (Data) {
    ...
    Data.groupedItems = list.createGrouped(function groupKeySelector(item) {
    ...
    Data.groups = Data.groupedItems.groups;
    ...
    function resolveGroupReference(key) {
        return Data.groupedItems.groups.getItemFromKey(key).data;
    }
    Data.resolveGroupReference = resolveGroupReference;

    // Get a unique item from the provided string array, which should contain a
    // group key and an item title.
    function resolveItemReference(reference) {
        for (var i = 0; i < Data.groupedItems.length; i++) {
            var item = Data.groupedItems.getAt(i);
            if (item.group.key === reference[0] && item.title === reference[1]) {
                return item;
            }
        }
    }
    Data.resolveItemReference = resolveItemReference;
